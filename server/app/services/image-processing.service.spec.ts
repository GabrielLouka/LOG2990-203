...
describe('Image-Processing Service', () => {
    ...
    it('should sets the RGB values of a pixel in the image buffer', () => {
      ...
      it('should return a null and throw an error', () => {
        ...
        expect(spy.callCount).to.equal(2);
        expect(spy.getCall(0).args[0]).to.be.an('error');
        expect(spy.getCall(1).args[0]).to.equal("OOPS! Couldn't get the RGB values for the pixel at position "
        + invalidPosition.x + ', ' + invalidPosition.y + '!');
    ...
    it('should return the RGB values of a pixel in the image buffer', () => {
        ...
        it('should return the positions differences in an array', ()=>{
            const radius=3;
            const expected:Vector2[][]=[[ {x: 4,y: 0},{x: 3,y: 0},{x: 2,y: 0},{x: 1,y: 0},{x: 0,y: 0}]];
            const imageBuffer1=Buffer.alloc(3);
            const imageBuffer2=Buffer.alloc(3);
            ...
            const getDifferentPixelPositionsBetweenImagesStub:sinon.SinonStub =
             sinon.stub(imageProcessingService, <any>'getDifferentPixelPositionsBetweenImages');
        ...
        it('should return an image with all differences in black and white', ()=>{
            ...
            const radius=3;
            const allDifferences:Vector2[][]=[[ {x: 4,y: 0},{x: 3,y: 0},{x: 2,y: 0},{x: 1,y: 0},{x: 0,y: 0}]];
            for (let i =0; i<imageBuffer1.length;i++){
                imageBuffer1.writeUInt8(5,i);
                imageBuffer2.writeUInt8(6,i);
            ...
            expect(result).to.equal({
                ...
                message: 'Success!',
                generatedGameId: -1,
                differences: [[ {x: 4,y: 0},{x: 3,y: 0},{x: 2,y: 0},{x: 1,y: 0},{x: 0,y: 0}]],
                isEasy: true
              });
        });
