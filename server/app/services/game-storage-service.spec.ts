...
describe('Game storage service', () => {
    ...
    it('should update a game name of a specific game', async ()=>{
        const newName='newGlouton';
        const id=5;
        const updateGame=await gameStorageService.updateGameName(id,newName);
        expect(updateGame.acknowledged).to.equals(true);
    });
   ...
   it('should store the game result', async()=>{
        gameStorageService.storeGameResult(gamePrototype.id,gamePrototype.differences,gamePrototype.isEasy);
        const allGames=await gameStorageService.getAllGames();
   ...
   it('should return the next available game id', ()=>{
    ...
    it('should throw an error when creating the folder', ()=>{
        ...
        mkdirStub.callsFake((path, options, callback) => {
    ...
    describe('Error Handling', async ()=>{
        ...
        it('should throw an error if we try to get a specific game on a closed connection', async ()=>{
            await client.close();
            expect(gameStorageService.updateGameName(5,'naruto')).to.eventually.be.rejectedWith(Error);
        });
    });
});
