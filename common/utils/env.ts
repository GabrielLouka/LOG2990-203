const MAX_PEN_WIDTH = 20;
const PEN_WIDTH = 10;
const DEFAULT_ENLARGEMENT_RADIUS = 3;
const MAX_GAMES_PER_PAGE = 4;
const NOT_FOUND = -1;
const CANVAS_WIDTH = 640;
const CANVAS_HEIGHT = 480;
const LIMITED_TIME_DURATION = 120;
const MILLISECOND_TO_SECONDS = 1000;
const REPLAY_TIMER_DELAY = 50;
const QUARTER_SECOND = 250;
const MINUTE_TO_SECONDS = 60;
const THIRTY_MINUTES_TO_SECONDS = 1800;
const THIRTY_ONE_MINUTES_TO_SECONDS = 1860;
const THIRTY_TWO_MINUTES_TO_SECONDS = 1920;
const MINUTE_LIMIT = 10;
const IMAGE_WIDTH_OFFSET = 18;
const IMAGE_HEIGHT_OFFSET = 22;
const BLINK_TIME = 100;
const NUMBER_OF_BLINKS = 3;
const NUMBER_HINTS = 3;
const BMP_FILE_HEADER_BYTES_LENGTH = 54;
const PIXEL_BYTES_LENGTH = 3;
const MIN_NBR_OF_DIFFERENCES = 3;
const MAX_NBR_OF_DIFFERENCES = 9;
const MIN_HARD_DIFFERENCES = 7;
const REQUIRED_SURFACE_PERCENTAGE = 0.15;
const WAITING_FOR_PLAYER_TEXT = "En attente d'un adversaire...";
const WAITING_PLAYER_ANSWER_TEXT = "En attente de la réponse de l'adversaire...";
const DO_YOU_WANT_TO_PLAY_WITH_TEXT = 'Voulez-vous jouer avec ';
const REDIRECTED_TO_MAIN_PAGE_TEXT = "Le jeu a été supprimé, vous avez donc été redirigé à l'accueil";
const ALL_GAMES_TEXT = 'TOUS LES JEUX ?';
const THIS_GAME_TEXT = 'CE JEU ?';
const DELETE_TEXT = 'SUPPRIMER ';
const RESET_TEXT = 'RÉINITIALISER ';
const YES_TEXT = 'OUI';
const NO_TEXT = 'NON';
const LIMITED_TIME_USER_ENTERED = 'Pseudo validé, veuillez choisir un mode (solo ou co-op):';
const WAITING_FOR_PLAYER_MESSAGE = "En attente d'un adversaire...";
const WAITING_PLAYER_ANSWER_MESSAGE = "En attente de la réponse de l'adversaire...";
const DO_YOU_WANT_TO_PLAY_WITH_MESSAGE = 'Voulez-vous jouer avec ';
const ABORTED_GAME_MESSAGE = ' a abandonné la partie';
const CHAT_TITLE = 'MANIA CHAT';
const SYSTEM_NAME = 'System';
const ABORTED_GAME_TEXT = ' a abandonné la partie';
const VOLUME_ERROR = 0.3;
const VOLUME_SUCCESS = 1;
const ROUTE_TO_SENDING_IMAGE = '/image_processing/send-image';
const INITIAL_COUNTDOWN = 45;
const INITIAL_PENALTY = 5;
const INITIAL_BONUS = 5;
const MAX_PENALTY = 10;
const MAX_BONUS = 10;
const SPEED_X1 = 1;
const SPEED_X2 = 2;
const SPEED_X4 = 4;

export { SPEED_X1, SPEED_X2, SPEED_X4 };
export {
    DO_YOU_WANT_TO_PLAY_WITH_MESSAGE,
    WAITING_FOR_PLAYER_MESSAGE,
    WAITING_PLAYER_ANSWER_MESSAGE,
    LIMITED_TIME_USER_ENTERED,
    ABORTED_GAME_MESSAGE,
};
export { INITIAL_COUNTDOWN, INITIAL_PENALTY, INITIAL_BONUS, MAX_PENALTY, MAX_BONUS };
export { CHAT_TITLE };
export { PEN_WIDTH, MAX_PEN_WIDTH };
export { BMP_FILE_HEADER_BYTES_LENGTH, PIXEL_BYTES_LENGTH };
export { NOT_FOUND };
export { DO_YOU_WANT_TO_PLAY_WITH_TEXT, WAITING_FOR_PLAYER_TEXT, WAITING_PLAYER_ANSWER_TEXT, REDIRECTED_TO_MAIN_PAGE_TEXT, ABORTED_GAME_TEXT };
export { ALL_GAMES_TEXT, THIS_GAME_TEXT, DELETE_TEXT, RESET_TEXT, YES_TEXT, NO_TEXT };
export { CANVAS_HEIGHT, CANVAS_WIDTH };
export { MILLISECOND_TO_SECONDS, QUARTER_SECOND, MINUTE_TO_SECONDS, MINUTE_LIMIT, LIMITED_TIME_DURATION };
export { IMAGE_WIDTH_OFFSET, IMAGE_HEIGHT_OFFSET };
export { BLINK_TIME, NUMBER_OF_BLINKS };
export { SYSTEM_NAME };
export { MIN_NBR_OF_DIFFERENCES, MAX_NBR_OF_DIFFERENCES, MIN_HARD_DIFFERENCES };
export { REQUIRED_SURFACE_PERCENTAGE, DEFAULT_ENLARGEMENT_RADIUS };
export { THIRTY_MINUTES_TO_SECONDS, THIRTY_ONE_MINUTES_TO_SECONDS, THIRTY_TWO_MINUTES_TO_SECONDS };
export { VOLUME_ERROR };
export { VOLUME_SUCCESS, MAX_GAMES_PER_PAGE };
export { ROUTE_TO_SENDING_IMAGE };
export { NUMBER_HINTS };
export { REPLAY_TIMER_DELAY };
